...
#else
#if defined(ANDROID)
// Disable environment reflections on mobile to remove shimmering artifacts and save GPU
float fEnvScale               = 0.0f;
#else
...
uint8 nWaterAlpha             =
#if defined(ANDROID) && defined(ANDROID_REALISTIC_WATER)
	224;
#else
	192;
#endif
...
#ifdef ANDROID
	if (gpWaterEnvTex)
		RwTextureSetFilterMode(gpWaterEnvTex, rwFILTERLINEAR);
	if (gpWaterEnvBaseTex)
		RwTextureSetFilterMode(gpWaterEnvBaseTex, rwFILTERLINEAR);
	if (gpWaterTex)
		RwTextureSetFilterMode(gpWaterTex, rwFILTERLINEAR);
#endif
...
#if defined(ANDROID) && defined(ANDROID_REALISTIC_WATER)
		RwRGBA watercolor = { 220, 236, 255, 224 };
#else
...
#endif
#if defined(ANDROID) && defined(ANDROID_REALISTIC_WATER)
		RwRGBA watercolor = { 200, 228, 255, 216 };
#else
		RwRGBA watercolor = { 255, 255, 255, 192 };
#endif
...
{
	#ifdef ANDROID
	if     ( TheCamera.GetPosition().z < 15.0f  ) return 1200.0f * ANDROID_WATER_DRAW_DISTANCE_SCALE;
	if     ( TheCamera.GetPosition().z > 60.0f  ) return 2000.0f * ANDROID_WATER_DRAW_DISTANCE_SCALE;
	return (( TheCamera.GetPosition().z + -15.0f ) * 800.0f / 45.0f + 1200.0f) * ANDROID_WATER_DRAW_DISTANCE_SCALE;
	#else
...
_GetWavyDrawDist()
{
	...
	if ( FindPlayerVehicle() && FindPlayerVehicle()->IsBoat() )
		return 120.0f * ANDROID_WAVY_WATER_DRAW_DISTANCE_SCALE;
	else
		return 70.0f * ANDROID_WAVY_WATER_DRAW_DISTANCE_SCALE;
	#else
	if ( FindPlayerVehicle() && FindPlayerVehicle()->IsBoat() )
		return 120.0f;
	else
...
#ifndef PC_WATER
#if defined(ANDROID) && defined(ANDROID_REALISTIC_WATER)
			maskPreLight[base+a].red   = Min(255, (int32)(maskPreLight[base+a].red * 0.86f));
			maskPreLight[base+b].red   = Min(255, (int32)(maskPreLight[base+b].red * 0.86f));
			maskPreLight[base+c].red   = Min(255, (int32)(maskPreLight[base+c].red * 0.86f));
			maskPreLight[base+d].red   = Min(255, (int32)(maskPreLight[base+d].red * 0.86f));
			maskPreLight[base+a].green = Min(255, (int32)(maskPreLight[base+a].green * 0.95f));
			maskPreLight[base+b].green = Min(255, (int32)(maskPreLight[base+b].green * 0.95f));
			maskPreLight[base+c].green = Min(255, (int32)(maskPreLight[base+c].green * 0.95f));
			maskPreLight[base+d].green = Min(255, (int32)(maskPreLight[base+d].green * 0.95f));
			maskPreLight[base+a].blue  = Min(255, (int32)(maskPreLight[base+a].blue * 1.14f));
			maskPreLight[base+b].blue  = Min(255, (int32)(maskPreLight[base+b].blue * 1.14f));
			maskPreLight[base+c].blue  = Min(255, (int32)(maskPreLight[base+c].blue * 1.14f));
			maskPreLight[base+d].blue  = Min(255, (int32)(maskPreLight[base+d].blue * 1.14f));
#endif
