...
bool CPed::CheckObjectAbovePlayer(CColPoint& hitForwardPoint)
{
    ...
    bool isObjectAbovePlayer = CWorld::ProcessLineOfSight(GetPosition(),
        GetPosition() + CVector(0.0f, 0.0f, 2.75f),
        colPoint,
        hitEntity,
    ...
    return isObjectAbovePlayer || hitObjectAbovePlayer1 || hitObjectAbovePlayer2 ||
           hitObjectAbovePlayer3 || hitObjectAbovePlayer4 || hitObjectAbovePlayer5;
}
...
void CPed::StartClimbing(CColPoint& hitForwardPoint, CColPoint& hitBackwardPoint, CColPoint& hitJumpBPoint)
{
	...
	if (currentClimbingHeight > maxHighClimbingHeight && !bIsClimbingJumpB) {
		...
		CAnimBlendAssociation* idleClimbAssoc = CAnimManager::AddAnimation(GetClump(), ASSOCGRP_STD, ANIM_STD_CLIMBING_JUMP_JUMP);
		...
		if (bIsClimbingJumpB) {
			if (bIsClimbingHighJump) {
				...
				currentClimbingJumpBAnim = CAnimManager::AddAnimation(GetClump(), ASSOCGRP_STD, ANIM_STD_CLIMBING_JUMP_JUMP_B);
			...
			currentClimbingStandAnim = CAnimManager::AddAnimation(GetClump(), ASSOCGRP_STD, ANIM_STD_CLIMBING_JUMP_STAND);
	...
	if (bIsClimbing) {
		...
		if (bIsClimbingIdle) {
			...
			if (!IsPlayer() || (pad0->JumpJustDown() && IsPlayer())) {
				...
				CAnimBlendAssociation* lowClimbAssoc = CAnimManager::AddAnimation(GetClump(), ASSOCGRP_STD, ANIM_STD_CLIMBING_JUMP_PULL);
...
void CPed::FinishHighClimbingCB(CAnimBlendAssociation* animAssoc, void* arg)
{
	...
	ped->currentClimbingIdleAnim = CAnimManager::BlendAnimation(ped->GetClump(), ASSOCGRP_STD, ANIM_STD_CLIMBING_JUMP_IDLE, 10.0f);
}
...
void CPed::FinishClimbingPullCB(CAnimBlendAssociation* animAssoc, void* arg)
{
	...
	if (ped->bIsClimbingJumpB) {
		ped->currentClimbingJumpBAnim = CAnimManager::AddAnimation(ped->GetClump(), ASSOCGRP_STD, ANIM_STD_CLIMBING_JUMP_JUMP_B);
		ped->currentClimbingJumpBAnim->SetFinishCallback(FinishClimbingCB, ped);
	} else {
		ped->currentClimbingStandAnim = CAnimManager::AddAnimation(ped->GetClump(), ASSOCGRP_STD, ANIM_STD_CLIMBING_JUMP_STAND);
...
CPed::FinishClimbingCB(CAnimBlendAssociation *animAssoc, void *arg)
{
	...
	if (ped->bIsClimbing)
	{
		if (!ped->bIsClimbingJumpB) {
			...
			CAnimBlendAssociation* lowClimbFinishAnim = CAnimManager::AddAnimation(ped->GetClump(), ASSOCGRP_STD, ANIM_STD_CLIMBING_JUMP_STAND_FINISH);
