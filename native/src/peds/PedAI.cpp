...
CPed::SetAnimOffsetForEnterOrExitVehicle(void)
{
	...
	auto SetAnimTranslationOffset = [](CVector &outOffset, AssocGroupId group, AnimationId animId) {
		outOffset = CVector(0.0f, 0.0f, 0.0f);
		CAnimBlendAssociation *assoc = CAnimManager::GetAnimAssociation(group, animId);
		if (assoc == nil || assoc->hierarchy == nil)
			return;
		CAnimBlendHierarchy *enterAssoc = assoc->hierarchy;
		CAnimBlendSequence *seq = enterAssoc->sequences;
		CAnimManager::UncompressAnimation(enterAssoc);
		if (seq != nil && seq->numFrames > 0 && seq->HasTranslation()) {
			KeyFrameTrans *lastFrame = (KeyFrameTrans*)seq->GetKeyFrame(seq->numFrames - 1);
			if (lastFrame != nil)
				outOffset = lastFrame->translation;
		}
	};
	...
	SetAnimTranslationOffset(vecPedDraggedOutCarAnimOffset, ASSOCGRP_STD, ANIM_STD_JACKEDCAR_LHS);
	SetAnimTranslationOffset(vecPedCarDoorAnimOffset, ASSOCGRP_STD, ANIM_STD_CAR_GET_IN_LHS);
	SetAnimTranslationOffset(vecPedCarDoorLoAnimOffset, ASSOCGRP_STD, ANIM_STD_CAR_GET_IN_LO_LHS);
	SetAnimTranslationOffset(vecPedQuickDraggedOutCarAnimOffset, ASSOCGRP_STD, ANIM_STD_QUICKJACKED);
	SetAnimTranslationOffset(vecPedVanRearDoorAnimOffset, ASSOCGRP_VAN, ANIM_STD_VAN_GET_IN_REAR_LHS);
	SetAnimTranslationOffset(vecPedTrainDoorAnimOffset, ASSOCGRP_STD, ANIM_STD_TRAIN_GETOUT);
	SetAnimTranslationOffset(vecPedStdBikeJumpRhsAnimOffset, ASSOCGRP_BIKE_STANDARD, ANIM_BIKE_JUMPON_LHS);
	SetAnimTranslationOffset(vecPedVespaBikeJumpRhsAnimOffset, ASSOCGRP_BIKE_VESPA, ANIM_BIKE_JUMPON_LHS);
	SetAnimTranslationOffset(vecPedHarleyBikeJumpRhsAnimOffset, ASSOCGRP_BIKE_HARLEY, ANIM_BIKE_JUMPON_LHS);
	SetAnimTranslationOffset(vecPedDirtBikeJumpRhsAnimOffset, ASSOCGRP_BIKE_DIRT, ANIM_BIKE_JUMPON_LHS);
	SetAnimTranslationOffset(vecPedBikeKickAnimOffset, ASSOCGRP_BIKE_HARLEY, ANIM_BIKE_KICK);
