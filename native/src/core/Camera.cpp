...
#ifdef ANDROID
	if (FindPlayerVehicle())
		GenerationDistMultiplier *= ANDROID_VEHICLE_GENERATION_DISTANCE_SCALE;
	else
		GenerationDistMultiplier *= ANDROID_GENERATION_DISTANCE_SCALE;
#endif
...
#ifdef ANDROID
				if (m_bEnable1rstPersonCamCntrlsScript && (CPad::GetPad(0)->LookAroundLeftRight() || CPad::GetPad(0)->LookAroundUpDown())) {
