...
void CPad::UpdateTouch() // 1wn_klaymen1n: I know it's better to use some preprocessor directives for this, but I'm lazy ass.
{
	int32 signX = 1;
	int32 signY = 1;
...
#ifdef ANDROID
	if (FindPlayerVehicle()) {
		bool left = gTouch.getButton(BtnType::CAR_LEFT);
		bool right = gTouch.getButton(BtnType::CAR_RIGHT);
		if (left != right)
			return left ? -128 : 128;
	}
#endif
...
#ifdef ANDROID
	if (FindPlayerVehicle())
		return gTouch.getButton(BtnType::CAR_BRAKE) ? 255 : 0;
#endif
...
int32 CPad::GetWeapon(void)
{
	...
	if (gTouch.getButton(BtnType::FIGHT))
		return true;
...
bool CPad::WeaponJustDown(void)
{
	...
	if (gTouch.getButtonJustDown(BtnType::FIGHT))
		return true;
...
#ifdef ANDROID
	if (FindPlayerVehicle())
		return gTouch.getButton(BtnType::CAR_ACCEL) ? 255 : 0;
#endif
...
bool CPad::JumpJustDown(void)
{
	...
	if (gTouch.getButtonJustDown(BtnType::JUMP)) {
		static uint32 lastSprintTapTime = 0;
		uint32 now = CTimer::GetTimeInMilliseconds();
		bool doubleTap = now - lastSprintTapTime <= 250;
...
{
#ifdef ANDROID
	if (gTouch.lookAxisX != 0 && !GetLookBehindForPed())
		return Clamp((int32)gTouch.lookAxisX, -127, 127);
#endif
...
#ifdef ANDROID
	if (gTouch.lookAxisY != 0 && !GetLookBehindForPed())
		return Clamp((int32)axis, -127, 127);
#endif
