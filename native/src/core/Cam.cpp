...
CCam::Process(void)
{
	...
	//	Process_TopDown(CameraTarget, TargetOrientation, SpeedVar, TargetSpeedVar);
		break;
	case MODE_BEHINDCAR:
        Process_CarWithMouse(CameraTarget, TargetOrientation, SpeedVar, TargetSpeedVar);
	...
	case MODE_FOLLOWPED:
		...
		break;
#elif defined(PC_PLAYER_CONTROLS) //HACK we need a special function for touch, right?
...
float fPedMouseTranslateUp = 0.8f;
float fPedSwipeSensitivity = 0.03f;
float fPedSwipeSmoothness = 0.9f;
float fPedMaxBeta = DEGTORAD(120.0f);
float fPedMaxAlpha = DEGTORAD(45.0f);
float fPedMinAlpha = -DEGTORAD(60.0f);

float fCarMouseBaseDist = 3.5f;
float fCarMouseAngleDist = 2.0f;
float fCarMouseFalloff = 3.0f;
float fCarMouseTranslateUp = 1.0f;

float fCarSwipeSensitivity = 0.025f;
float fCarSwipeSmoothness = 0.92f;
float fCarMaxBeta = DEGTORAD(120.0f);

float fCarAutoRotateStrength = 0.12f;
float fCarAutoRotateSpeed = 0.05f;

float fCarSpeedMult = 2.5f;
float fCarSteerMult = 3.2f;

float fCarMouseRotateSpeed = 0.006f;
float fCarMouseRotateAccel = 0.009f;
float fCarMouseReturnSpeed = 0.012f;
float fCarMaxRotateAngle = DEGTORAD(8.0f);

void
CCam::Process_CarWithMouse(const CVector &CameraTarget, float TargetOrientation, float, float)
{
    static bool firstTime = true;
    static float currentRotateAngle = 0.0f;
    static float rotateSpeed = 0.0f;
    if(firstTime) {
        firstTime = false;
        debug("Process_CarWithMouse called!\n");
    }
    ...
    TargetCoors = CameraTarget;
    TargetCoors.z += fCarMouseTranslateUp;

    float AlphaOffset = 0.0f, BetaOffset = 0.0f;
    float MouseBetaOffset = 0.0f, MouseAlphaOffset = 0.0f;
    ...
    if(CPad::GetPad(0)->IsPlayerControlsDisabledBy(PLAYERCONTROL_PLAYERINFO)) {
        ...
        if(UseMouse) {
            MouseBetaOffset = LookLeftRight * TheCamera.m_fMouseAccelHorzntl * FOV / 80.0f;
            MouseAlphaOffset = LookUpDown * TheCamera.m_fMouseAccelVertical * FOV / 80.0f;
        } else {
            MouseBetaOffset = LookLeftRight * fStickSens * (1.0f / 14.0f) * FOV / 80.0f * CTimer::GetTimeStep();
            MouseAlphaOffset = LookUpDown * fStickSens * (0.6f / 14.0f) * FOV / 80.0f * CTimer::GetTimeStep();
        }
    }

    CVehicle* car = (CVehicle*)CamTargetEntity;
    CVector carVel = car->GetMoveSpeed();
    float forwardSpeed = DotProduct(carVel, car->GetForward());
    float carSpeed = Abs(forwardSpeed);
    float steeringAngle = 0.0f;

    if(car->IsCar()) {
        steeringAngle = ((CAutomobile*)car)->m_fSteerAngle;
    }
    ...
    bool isTurning = (Abs(steeringAngle) > 0.08f) && (carSpeed > 0.35f);

    if(isTurning) {
        float speedFactor = Min(carSpeed * 8.0f, 1.0f);
        float steerFactor = Min(Abs(steeringAngle) * 4.0f, 1.0f);
        float turnStrength = speedFactor * steerFactor;

        if(steeringAngle > 0)
            targetRotateAngle = -turnStrength * fCarMaxRotateAngle;
        else
            targetRotateAngle = turnStrength * fCarMaxRotateAngle;
    }

    if(isTurning && !UseMouse && Abs(LookLeftRight) < 0.001f) {
        if(Abs(currentRotateAngle - targetRotateAngle) > 0.001f) {
            WellBufferMe(targetRotateAngle, &currentRotateAngle, &rotateSpeed,
                         fCarMouseRotateSpeed, fCarMouseRotateAccel, false);
        }
        BetaOffset += currentRotateAngle;
    } else {
        if(Abs(currentRotateAngle) > 0.001f) {
            WellBufferMe(0.0f, &currentRotateAngle, &rotateSpeed,
                         fCarMouseReturnSpeed * 1.8f, fCarMouseRotateAccel, false);
            BetaOffset += currentRotateAngle;
        } else {
            currentRotateAngle = 0.0f;
            rotateSpeed = 0.0f;
    ...
    {
        float referenceBeta = TargetOrientation + PI + currentRotateAngle;
        float newBeta = WrapAngle(Beta + MouseBetaOffset);
        float ref = WrapAngle(referenceBeta);
        float deltaFromRef = WrapAngle(newBeta - ref);
        float manualLimit = DEGTORAD(75.0f);
        deltaFromRef = Clamp(deltaFromRef, -manualLimit, manualLimit);
        float limitedBeta = WrapAngle(ref + deltaFromRef);
        float limitedOffset = WrapAngle(limitedBeta - Beta);
        BetaOffset += limitedOffset;
        AlphaOffset = MouseAlphaOffset;
    }
    ...
    if(Alpha > DEGTORAD(70.0f)) Alpha = DEGTORAD(70.0f);
    else if(Alpha < -DEGTORAD(70.0f)) Alpha = -DEGTORAD(70.0f);

    float clampedAlpha = Min(Alpha * fCarMouseFalloff, HALFPI);
    CamDist = fCarMouseBaseDist + Cos(clampedAlpha) * fCarMouseAngleDist;

    CColModel *colModel = CamTargetEntity->GetColModel();
    if(colModel) {
        float carSize = colModel->boundingBox.GetSize().Magnitude();
        CamDist += carSize * 0.2f;
    }
    ...
    if(CWorld::ProcessLineOfSight(TargetCoors, Source, colPoint, entity,
        ...
        float CarColDist = (TargetCoors - colPoint.point).Magnitude();
        float ColCamDist = CamDist - CarColDist;

        if(entity->IsPed() && ColCamDist > DEFAULT_NEAR + 0.1f) {
            if(CWorld::ProcessLineOfSight(colPoint.point, Source, colPoint, entity,
                                          true, true, true, true, false, false, true)) {
                CarColDist = (TargetCoors - colPoint.point).Magnitude();
                Source = colPoint.point;
                if(CarColDist < DEFAULT_NEAR + 0.3f)
                    RwCameraSetNearClipPlane(Scene.camera, Max(CarColDist - 0.3f, 0.05f));
            } else {
    ...
    entity = CWorld::TestSphereAgainstWorld(Source + Front * Near, radius,
                                            nil, true, true, false, true, false, false);
...
CCam::Process_FollowPedWithMouse(const CVector &CameraTarget, float TargetOrientation, float, float)
{
    ...
    TargetCoors = CameraTarget;
    TargetCoors.z += fPedMouseTranslateUp;

    static float targetBeta = Beta;
    static float currentBeta = Beta;
    static float targetAlpha = Alpha;
    static float currentAlpha = Alpha;
    ...
    if(Abs(MouseX) > 0.01f || Abs(MouseY) > 0.01f) {
        if(!isSwiping) {
            isSwiping = true;
            lastMouseX = MouseX;
            lastMouseY = MouseY;
            ...
            targetBeta += deltaX * fPedSwipeSensitivity * TheCamera.m_fMouseAccelHorzntl * 20.0f;
            targetAlpha += deltaY * fPedSwipeSensitivity * TheCamera.m_fMouseAccelVertical * 20.0f;

            while(targetBeta > PI) targetBeta -= 2 * PI;
            while(targetBeta < -PI) targetBeta += 2 * PI;

            targetAlpha = Clamp(targetAlpha, fPedMinAlpha, fPedMaxAlpha);
    ...
    float smoothFactor = Pow(fPedSwipeSmoothness, CTimer::GetTimeStep());
    currentBeta = targetBeta * (1.0f - smoothFactor) + currentBeta * smoothFactor;
    currentAlpha = targetAlpha * (1.0f - smoothFactor) + currentAlpha * smoothFactor;
    ...
    if(TheCamera.GetFading() && TheCamera.GetFadingDirection() == FADE_IN && nFadeControlThreshhold < CDraw::FadeValue ||
       ...
       CPad::GetPad(0)->IsPlayerControlsDisabledBy(PLAYERCONTROL_PLAYERINFO)) {
        ...
        float targetAlpha = -0.22f;

        if(Alpha < targetAlpha - 0.05f)
            Alpha = targetAlpha - 0.05f;
        else if(Alpha > targetAlpha + 0.05f)
            Alpha = targetAlpha + 0.05f;
    }

    float clampedAlpha = Min(Alpha * fFalloff, HALFPI);
    CamDist = fBaseDist + Cos(clampedAlpha) * fAngleDist;
...
CCam::Process_FollowCar_SA(const CVector& CameraTarget, float TargetOrientation, float, float)
{
	...
	bool hasManualLookInput = Abs(stickX) > 1.0f || Abs(stickY) > 1.0f;
	...
	if (!nextDirectionIsForward && !hasManualLookInput) {
		yMovement = 0.0f;
		xMovement = 0.0f;
	}
