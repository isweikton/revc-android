...
bool LoadINISettings()
{
	...
	bool hasMaxFPSSetting = ReadIniIfExists("Graphics", "MaxFPS", &RsGlobal.maxFPS);
	mINI::INIMap<std::string> displaySection = cfg.get("Display");
	bool hasDrawDistanceSetting = displaySection.has("DrawDistance");

#ifdef ANDROID
	if (!hasMaxFPSSetting || RsGlobal.maxFPS == 60)
		RsGlobal.maxFPS = ANDROID_DEFAULT_MAX_FPS;
	RsGlobal.maxFPS = Clamp(RsGlobal.maxFPS, 30, ANDROID_MAX_FPS_CAP);
	FrontEndMenuManager.m_PrefsFrameLimiter = 0;
	FrontEndMenuManager.m_PrefsVsync = 0;
	FrontEndMenuManager.m_PrefsVsyncDisp = 0;
	if (!hasDrawDistanceSetting || FrontEndMenuManager.m_PrefsLOD >= 1.19f)
		FrontEndMenuManager.m_PrefsLOD = ANDROID_DEFAULT_DRAW_DISTANCE;
	FrontEndMenuManager.m_PrefsLOD = Clamp(FrontEndMenuManager.m_PrefsLOD, 0.85f, 2.2f);
	if (CIniFile::PedNumberMultiplier == 0.6f)
		CIniFile::PedNumberMultiplier = ANDROID_DEFAULT_PED_DENSITY;
	if (CIniFile::CarNumberMultiplier == 0.6f)
		CIniFile::CarNumberMultiplier = ANDROID_DEFAULT_CAR_DENSITY;
	...
	CustomPipes::VehicleShininess = ANDROID_DEFAULT_VEHICLE_SHININESS;
	CustomPipes::VehicleSpecularity = 0.0f;
	CustomPipes::LightmapEnable = false;
	CustomPipes::GlossEnable = false;
	CustomPipes::RimlightEnable = false;
	CustomPipes::LightmapMult = ANDROID_DEFAULT_LIGHTMAP_MULT;
	CustomPipes::GlossMult = ANDROID_DEFAULT_GLOSS_MULT;
	CustomPipes::RimlightMult = ANDROID_DEFAULT_RIMLIGHT_MULT;
	CustomPipes::EnvMapSize = 4;
#endif
#ifdef SCREEN_DROPLETS
	ScreenDroplets::ms_enabled = false;
	ScreenDroplets::ms_movingEnabled = false;
#endif
#endif
