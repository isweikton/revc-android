...
#ifndef ONE_THREAD_PER_CHANNEL
	size_t allocBytes = ms_streamingBufferSize * CDSTREAM_SECTOR_SIZE;
	size_t capBytes = (size_t)ANDROID_STREAMING_MEMORY_CAP_MB * 1024 * 1024;
	if (allocBytes > capBytes) {
		ms_streamingBufferSize = (uint32)(capBytes / CDSTREAM_SECTOR_SIZE);
		if(ms_streamingBufferSize & 1) ms_streamingBufferSize--;
		allocBytes = ms_streamingBufferSize * CDSTREAM_SECTOR_SIZE;
	}
	ms_pStreamingBuffer[0] = (int8*)RwMallocAlign(allocBytes, CDSTREAM_SECTOR_SIZE);
	ms_streamingBufferSize /= 2;
	ms_pStreamingBuffer[1] = ms_pStreamingBuffer[0] + ms_streamingBufferSize*CDSTREAM_SECTOR_SIZE;
#else
	size_t allocBytes = ms_streamingBufferSize * 2 * CDSTREAM_SECTOR_SIZE;
	size_t capBytes = (size_t)ANDROID_STREAMING_MEMORY_CAP_MB * 1024 * 1024;
	if (allocBytes > capBytes) {
		ms_streamingBufferSize = (uint32)(capBytes / (2 * CDSTREAM_SECTOR_SIZE));
		if(ms_streamingBufferSize & 1) ms_streamingBufferSize--;
		allocBytes = ms_streamingBufferSize * 2 * CDSTREAM_SECTOR_SIZE;
	}
	ms_pStreamingBuffer[0] = (int8*)RwMallocAlign(allocBytes, CDSTREAM_SECTOR_SIZE);
	ms_streamingBufferSize /= 2;
	ms_pStreamingBuffer[1] = ms_pStreamingBuffer[0] + ms_streamingBufferSize*CDSTREAM_SECTOR_SIZE;
