...
#elif defined(PC_PLAYER_CONTROLS)
...
CMenuManager::DrawBackground(bool transitionCall)
{
	...
	if (m_bShowMouse) {
#ifndef ANDROID_HIDE_FRONTEND_CURSOR
...
CMenuManager::UserInput(void)
{
	...
	for (int rowToCheck = 0; rowToCheck < NUM_MENUROWS; ++rowToCheck) {
		...
		//1wn_klaymen1n: now we are checking X too
	...
	if (hasNativeList(m_nCurrScreen)) {
		...
		if (hoveredCoords.isValid && CheckHover(hoveredCoords.x1, hoveredCoords.x2, hoveredCoords.y1, hoveredCoords.y2)){ //1wn_klaymen1n: still checking
		...
		switch (option) {
			...
			case MENUACTION_RESTOREDEF:
				if (m_nCurrScreen == MENUPAGE_SOUND_SETTINGS) {
					...
					m_PrefsBrightness = 256;
#ifdef ANDROID
					m_PrefsLOD = ANDROID_DEFAULT_DRAW_DISTANCE;
#else
					m_PrefsLOD = 1.2f;
#endif
#ifdef LEGACY_MENU_OPTIONS
					...
					m_ControlMethod = CONTROL_STANDARD;
#if defined(PC_PLAYER_CONTROLS) && !defined(ANDROID)
