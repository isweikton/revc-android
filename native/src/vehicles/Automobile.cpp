...
CAutomobile::PreRender(void)
{
	...
	// .. ебаная хуета которая дает блик на авто типо от солнца, но сука даже ночью
	// .. DoSunGlare();
		...
		if(m_modelIndex == MI_SEASPAR && impulseRatio < 3.0f && (GetUp().z > -0.5f || impulseRatio < 0.6f) ||
		   CVehicle::bHoverCheat && GetStatus() == STATUS_PLAYER && GetUp().z > 0.1f){
			...
			if(pDriver){
				...
				if(
#if defined(ANDROID) && defined(ANDROID_DISABLE_PLAYER_DROWNING)
					!pDriver->IsPlayer() &&
#endif
					(pDriver->IsPlayer() || !bWaterTight))
			...
			for(i = 0; i < m_nNumMaxPassengers; i++)
				if(pPassengers[i]){
					...
					if(
#if defined(ANDROID) && defined(ANDROID_DISABLE_PLAYER_DROWNING)
						!pPassengers[i]->IsPlayer() &&
#endif
						(pPassengers[i]->IsPlayer() || !bWaterTight))
