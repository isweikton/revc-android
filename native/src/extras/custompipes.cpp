...
int32 VehiclePipeSwitch = VEHICLEPIPE_MATFX;
 #ifdef ANDROID
float VehicleShininess = ANDROID_DEFAULT_VEHICLE_SHININESS;
 #else
...
bool LightmapEnable;
#ifdef ANDROID
float LightmapMult = ANDROID_DEFAULT_LIGHTMAP_MULT;
#else
...
bool GlossEnable;
#ifdef ANDROID
float GlossMult = ANDROID_DEFAULT_GLOSS_MULT;
#else
...
bool RimlightEnable;
#ifdef ANDROID
float RimlightMult = ANDROID_DEFAULT_RIMLIGHT_MULT;
#else
